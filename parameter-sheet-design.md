...
> ※ **サーバLANのデフォルトゲートウェイを追加（2026-07-23・オーナー指示）**。`segments[0]` に固定キー
>   **`sv_lan_gateway`**（サーバLAN デフォルトゲートウェイ・**sv_lan専用**）を追加する。
>   - 代入方式: 値代入（文字列。IPv4アドレス形式）。空欄/未登録なら「ゲートウェイを設定しない」に倒す（[int]変換はしない）。
>   - **管理LAN（mgmt_lan）/ バックアップLAN（bk_lan）はゲートウェイ列を持たない**（＝空白。ゲートウェイを設定しない）。
>     デフォルトゲートウェイは sv_lan に一本化する。
>   - パラメータシート項目（追加）: 「サーバLAN デフォルトGW」→ `VAR_vm[n].segments[0].sv_lan_gateway`（△・sv_lanのみ）。
